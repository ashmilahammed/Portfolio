...
document.getElementById('form').addEventListener('submit', function(event) {
  ...
  // Check if name is empty or is only a number
  if (name === '') {
      showError('name', 'Name is required.');
      isValid = false;
  } else if (isNumber(name)) {
      showError('name', 'Name cannot be a number.');
      isValid = false;
  }
  ...
  emailjs.send("service_o0hebh9", "template_8awtha9", {
      name: name,
      email: email,  // Ensure this matches the placeholder in the template
      message: message
  })
